...
#' @param predictionIntervals A vector with confidence levels for prediction intervals. Default is c(80,90).
#' @returns The forecasted values for the original non-stationary, not seasonally adjusted series with prediction intervals.
#' @examples
#' library(urca)
#' library (forecast)
#' library(wooldridge)
...
#' Tbill3M_stationary=stationarize(ts(Tbill3M_sa, start=c(1960,1), frequency=4), type=type)$x_stationary
...
forecastOriginal_quarterly=function(arimaEstimation, type, seasonalCoeff, seasonalityType="none", nQuarters=4, year, quarter, saSeries, predictionIntervals=c(80,90))
{
  ...
  forecast_x_sa_stationary_ts=list()
  forecast_x_sa=list()
  forecast_diff_x_sa=list()
  forecast_diff_x_sa_ts=list()
  forecast_x=list()

  for(series in 1:(length(predictionIntervals)*2+1))
  {
    forecast_x_sa_stationary_ts[[series]]=ts(forecast_x_sa_stationary[,series], start=c(year,quarter), frequency=4)
    ...
    if(type=="Stationary, no trend")
    {
      ...
      forecast_x_sa[[series]]=forecast_x_sa_stationary_ts[[series]]
    } else {
      if(type=="Stationary around a linear trend")
      {
        ...
        forecast_x_sa[[series]]=intercept + betaTrend*trendF + forecast_x_sa_stationary_ts[[series]]
        forecast_x_sa[[series]]=ts(forecast_x_sa[[series]], start=c(year,quarter), frequency=4)
      } else {
        if(type=="Random walk with drift")
        {
          ...
          forecast_x_sa[[series]]=numeric()
          forecast_x_sa[[series]][1]=x_sa_ts[length(x_sa_ts)] + forecast_x_sa_stationary_ts[[series]][1]
          for (i in 2:length(forecast_x_sa_stationary_ts[[series]]))
          {
            forecast_x_sa[[series]][i]=forecast_x_sa[[series]][i-1] + forecast_x_sa_stationary_ts[[series]][i]
          }
          forecast_x_sa[[series]]=ts(forecast_x_sa[[series]], start=c(year,quarter), frequency=4)
        } else {
          if(type=="Unit root and linear trend")
          {
            ...
            trendF=(arimaEstimation$nobs+1):(arimaEstimation$nobs+nQuarters)
            forecast_diff_x_sa[[series]]=intercept + betaTrend*trendF + forecast_x_sa_stationary_ts[[series]]
            forecast_diff_x_sa_ts[[series]]=ts(forecast_diff_x_sa[[series]], start=c(year,quarter), frequency=4)
            ...
            forecast_x_sa[[series]]=numeric()
            forecast_x_sa[[series]][1]=x_sa_ts[length(x_sa_ts)] + forecast_diff_x_sa_ts[[series]][1]
            for (i in 2:length(forecast_diff_x_sa_ts[[series]]))
            {
              forecast_x_sa[[series]][i]=forecast_x_sa[[series]][i-1] + forecast_diff_x_sa_ts[[series]][i]
            }
            forecast_x_sa[[series]]=ts(forecast_x_sa[[series]], start=c(year,quarter), frequency=4)
          }
        }
    ...
    if(seasonalityType=="additive")
    {
      ...
      forecast_x[[series]]=forecast_x_sa[[series]] + seasonalCoeff_ts
      forecast_x[[series]]=as.data.frame(forecast_x[[series]])
    } else {
      if(seasonalityType=="multiplicative")
      {
        ...
        {
          forecast_x[[series]]=forecast_x_sa[[series]]
          forecast_x[[series]]=as.data.frame(forecast_x[[series]])
        }
      }
    }
  ...
  colnames(forecast_x_dataframe)[2:ncol(forecast_x_dataframe)]=colnames(forecast_x_sa_stationary)[2:ncol(forecast_x_sa_stationary)]
  forecast_x_dataframe
}
